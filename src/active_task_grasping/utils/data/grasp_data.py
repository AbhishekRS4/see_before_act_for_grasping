...
class GraspDatasetBase(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.include_depth:
            depth_img = self.get_depth_raw(idx, rot, zoom_factor)
            max_depth = np.max(depth_img)
            depth_img = depth_img.astype(np.float32) / max_depth
            depth_img = np.repeat(np.expand_dims(depth_img, -1), 3, -1)
            depth_img = (depth_img - self.depth_mean) / self.depth_std
            depth_img = np.transpose(depth_img, (2, 0, 1))

        # Load the RGB image
        if self.include_rgb:
            rgb_img = self.get_rgb_raw(idx, rot, zoom_factor)
            rgb_img = rgb_img.astype(np.float32) / 255.
            rgb_img = (rgb_img - self.color_mean) / self.color_std
        ...
        if self.include_depth and self.include_rgb:
            x = (self.numpy_to_torch(rgb_img), self.numpy_to_torch(depth_img))
